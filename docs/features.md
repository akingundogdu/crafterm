...
- **Fields** — project dropdown, base-branch dropdown (project's local branches, `main` preferred; disabled when git fails), **Local / Worktree** run-mode, **Labels** dropdown, prompt textarea, **Title + Branch name** inputs under it, **Build / Plan** toggle (Plan starts Claude with `--permission-mode plan`). **Cmd+Enter submits; Enter is a plain newline.** Draft text survives dropdown changes. **Verify:** each control persists its value across a re-render.
- **Title & Branch** — under the prompt: the title auto-fills with the prompt's first 20 characters while typed, until it is hand-edited (that breaks the prompt→title bond until the next submit; an emptied title falls back to the prompt head on submit). The branch box always follows the title as its slug — a hand-typed branch is slugified live (lowercase, invalid runs → `-`) but the next title change re-derives it. Its value is the slug suffix of the worktree branch (`KEY-<slug>`); the box is disabled in Local mode. **Verify:** type a prompt → title shows its first 20 chars, branch its slug; edit the title → branch re-slugs; keep typing the prompt → the edited title stays.
- **Labels dropdown** — next to the run-mode: a multi-select over the Daily Plan tags, ticked rows, caption showing the single name or an *N labels* count (full list in its tooltip). Closes on an outside click or Esc; empty state points at the board's Manage tags. The list is re-read every time the start screen is shown, and a tag deleted on the board drops out of the selection. **Verify:** pick two tags → caption reads "2 labels"; delete one on the board, reopen the composer → it is gone from the selection.
- **Slash menu** — typing `/` opens `/plan` `/build` `/local` `/worktree`, every sidebar project with a path, and every Daily Plan label; contains-first ranking (exact > prefix > mid-word), arrows + Enter/Tab to pick, Esc closes. Picking a label **toggles** it (a ✓ marks the ones already on the ticket) and the `/token` is cut back out of the prompt. **Verify:** type `/` → menu; type a project name → it ranks to the top; type `/urgent` + Enter → the urgent label is ticked in the Labels dropdown and the token disappears from the text.
- **Submit** — files a Daily Plan ticket for today (status *todo*, medium priority, carrying the picked labels as its tags, the branch slug as its `worktreeSlug`) in the selected project and opens a Claude terminal on it seeded with `ultrathink <ISSUE-KEY> <title>` — inside a worktree branched off the chosen base and named `KEY-<slug>`, or in the project itself. In worktree mode the terminal opens under the SAME name as the worktree (the branch, `KEY-<slug>`); in local mode it is titled by the ticket title. Refuses via dialog if no project is selected or the project has no issue-key prefix. **Verify:** pick a project with an issue-key prefix + Worktree + Cmd+Enter → ticket on the board with its labels, Claude terminal named exactly like its new worktree.
