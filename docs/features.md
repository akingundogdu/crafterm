...
- **Fields** — project dropdown, base-branch dropdown (project's local branches, `main` preferred; disabled when git fails), **Local / Worktree** run-mode, **Labels** dropdown, prompt textarea, **Build / Plan** toggle (Plan starts Claude with `--permission-mode plan`). **Cmd+Enter submits; Enter is a plain newline.** Draft text survives dropdown changes. **Verify:** each control persists its value across a re-render.
