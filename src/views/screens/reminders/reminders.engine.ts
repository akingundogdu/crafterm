...
import { settings, pushNotification, uid } from '@views/state/spine'
import { reminderRepo } from '@repositories'
import type { Reminder, ReminderPayload } from '@views/types/types'
...
// Reminders engine (gea tree, §2.7 self-contained). Merges the legacy
// reminders.state firing/scheduling logic with the reminders.tsx timer + snooze
// helpers. The reminderRepo stays the persisted source of truth; mutations refresh
// the reactive list via remindersStore.reload() instead of the legacy re-mount.
...
// card can render an Open action.
...
// then refresh the reactive list.
...
// the list as "past" (enabled=false, firedAt set) instead of being dropped. The
// reminderRepo objects are plain (not gea proxies), so the in-place mutation here
// is safe (§5.3 only bites on reactive proxies).
export function runTick(): void {
  ...
  if (changed) remindersStore.reload()
}

export function startReminderTimer(): void {
  window.setInterval(runTick, 20_000) // check every 20s
  runTick() // catch anything already due on launch
}
