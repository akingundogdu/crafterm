import type { LayoutNode, SidebarNode, DiffPane, CodePane } from '@ui/types/types'
...
import { firstPaneOf, allTabs, findById } from '@ui/tree/tree'
...
import { notebookNewNote, notebookNewFolder, notebookRenameSelected, notebookLinkFile, notebookSubTab } from '@ui/notebook/notebook'
import {
  openNewDailyTask,
  showDailyPlanModal,
  assignPaneToTask,
...
import { KEYBINDINGS, effectiveCombo, comboFromEvent, isRecording } from '@ui/keybindings/keybindings'
import { terminalService, claudeService, plansService, appService, storeService, deployService, paneService } from '@services'
import {
  newTab,
  newClaudeTab,
...
export function newTerminal(parentFolderId: string | null): void {
...
export function wirePaneActions(): void {
...
let quitting = false
export function wirePtyStream(): void {
...
export function installModalObserver(): void {
...
// ---- Periodic cwd/git refresh + Claude session-status refresh ----
...
export function wireServiceWatchers(): void {
...
export async function init(): Promise<void> {
  ...
  const wasUpdating = await deployService.wasUpdating()
