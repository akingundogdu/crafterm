...
export function createSelectionActions(handlers: SelectionActionsHandlers): HTMLDivElement {
  ...
  return (
    <div class="code-sel-actions" style="display: none">
      {h.onCopy && (
        <button class="code-sel-btn" onMousedown={h.preventCollapse} onClick={h.onCopy}>
          Copy
        </button>
      )}
      {h.onAddToChat && (
        <button class="code-sel-btn" onMousedown={h.preventCollapse} onClick={h.onAddToChat}>
          Add to Chat ⌘L
        </button>
      )}
    </div>
