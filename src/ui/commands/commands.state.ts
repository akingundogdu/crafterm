...
import { MAX_FOLDER_DEPTH } from '@ui/types/types'
import {
  panes,
  browsers,
  docs,
...
import { promptText, promptForm, promptConfirm, promptCloseActions } from '@ui/components/dialog/dialog'
...
async function liveCwd(paneId: string | null | undefined): Promise<string | undefined> {
  ...
  try {
    const info = await paneService.info(paneId)
    if (info?.cwd) {
