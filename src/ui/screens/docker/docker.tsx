...
export async function renderDocker(el: HTMLElement): Promise<void> {
  ...
  const bar = (<div class="docker-subtabs" />) as HTMLDivElement
  SUB_TABS.forEach((t) => {
    bar.appendChild(
      createButton({
...
function mergeStats(): void {
  ...
  host.querySelectorAll<HTMLElement>('.docker-row[data-stats-key]').forEach((row) => {
    ...
    if (!el) {
      el = (<div class="docker-row-stats" />) as HTMLDivElement
...
function appendPrune(list: HTMLElement, target: string, label: string): void {
  const foot = (<div class="docker-list-foot" />) as HTMLDivElement
  foot.appendChild(
    createButton({
      className: 'settings-inline-btn',
      text: label,
