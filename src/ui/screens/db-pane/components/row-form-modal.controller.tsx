...
export class RowFormModalController {
  ...
  open(): Promise<Record<string, FieldValue> | null> {
    return new Promise((resolve) => {
      ...
      const cancel = (
        <button onClick={() => this.close(null)}>{UITexts.DbPane.cancel}</button>
      ) as HTMLButtonElement
      const ok = (
        <button class="button-primary" onClick={() => this.close(collectFieldValues(this.opts.columns, this.inputs))}>
