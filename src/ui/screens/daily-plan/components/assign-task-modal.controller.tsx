...
export class AssignTaskModalController {
  ...
  open(): void {
    ...
    if (current) {
      const clear = (
        <button class="daily-plan-secondary-btn" onClick={() => this.assign(null)}>
          Clear
        </button>
    ...
    this.list = (<div class="daily-assign-list" />) as HTMLDivElement
    ...
    this.candidates = dailyTaskRepo.getAll()
  ...
  private renderList = (): void => {
    ...
    for (const t of items) {
      const row = (
        <button class="daily-assign-row" onClick={() => this.assign(t.id)}>
