  ...
  return (
    <div class="pane-box pane-popped">
      <div class="pane-popped-inner">
        ...
        <button class="settings-inline-btn" onClick={() => terminalService.popoutFocus(paneId)}>
          Focus window
        </button>
      </div>
    </div>
...
function buildNode(node: LayoutNode): HTMLElement {
  if (node.type === 'leaf') {
    ...
    if (!el) {
      el = (<div class="pane-box" />) as HTMLElement
    }
  ...
  const container = (<div class={'split ' + node.dir} />) as HTMLElement
  node.children.forEach((child, i) => {
    ...
    if (i < node.children.length - 1) {
      const rz = (<div class={'resizer ' + node.dir} />) as HTMLElement
...
function attachResizer(
  ...
  rz.addEventListener('mousedown', (e) => {
    ...
    const overlay = (<div class="resize-overlay" />) as HTMLElement
...
export function renderContent(): void {
  ...
  if (!entry) {
    const el = (<div class="tab-content" />) as HTMLElement
