...
export class ProjectsPanelController {
  ...
  build(): void {
    ...
    this.envBar = (<div class="env-bar" />) as HTMLDivElement
    ...
    this.groupBar = (<div class="env-bar" />) as HTMLDivElement
    panel.appendChild(this.groupBar)

    // ---- Catalog tree (left) + selected-project editor (right) ----
    this.listCol = (<div class="projects-md-list" />) as HTMLDivElement
    this.detailCol = (<div class="projects-md-detail" />) as HTMLDivElement
    const md = (
    ...
    buildSubTabs(detailCol, [
      {
        label: 'General',
        build: (el) => {
          this.field(el, 'Name', p.name, 'Movve', (v) => {
            p.name = v.trim()
            this.renderTree()
            requestSidebar()
            persistence.save()
          })
          this.field(el, 'Path', p.path, '~/code/movve', (v) => {
            p.path = v.trim()
            requestSidebar()
            persistence.save()
          })
          ...
          this.field(el, 'Command', p.command ?? '', 'claude (run on open, optional)', (v) => {
          ...
          this.field(el, 'Shell', p.shell ?? '', '/bin/zsh (override, optional)', (v) => {
      ...
      {
        label: 'Apps',
        build: (el) => buildAppsSection({ project: p, parent: el, field: this.field, uid, renderTree: this.renderTree, renderDetail: this.renderDetail })
      },
      {
        label: 'Features',
        build: (el) => buildFeaturesSection({ project: p, parent: el, uid, renderTree: this.renderTree, renderDetail: this.renderDetail })
      },
      {
        label: 'Run commands',
        build: (el) => buildRunCommandsSection({ project: p, parent: el, field: this.field, uid, renderDetail: this.renderDetail })
      },
